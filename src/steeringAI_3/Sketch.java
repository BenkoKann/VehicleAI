...
public class Sketch extends JPanel{
	...
	private static final int NFOOD = 40; // number of foods
	private static final int NPOISON = 20; // number of poisons
	private static final int NVEHICLES = 20;
	...
	protected void paintComponent(Graphics g) {
		...
		if(Helper.random(0, 100) < 5) { // 5 percent chance of new food spawn
			int x = (int) (Math.random() * WIDTH);
			int y = (int) (Math.random() * HEIGHT);
			food.add(new Vector(x, y));
		}
		
		if(Helper.random(0, 100) < 1) { // 1 percent chance of new poison spawn
			int x = (int) (Math.random() * WIDTH);
			int y = (int) (Math.random() * HEIGHT);
			poison.add(new Vector(x, y));
		}
